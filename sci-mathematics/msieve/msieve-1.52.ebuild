...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="zlib +ecm mpi cuda"

# some linking troubles with gwnum
DEPEND="
	ecm? ( sci-mathematics/gmp-ecm[-gwnum] )
	mpi? ( virtual/mpi )
	cuda? ( dev-util/nvidia-cuda-toolkit dev-util/nvidia-cuda-sdk )
	zlib? ( sys-libs/zlib )"
RDEPEND="${DEPEND}"
...
src_prepare() {
	...
	epatch \
		...
		"${FILESDIR}"/remove-svnversion.patch \
		"${FILESDIR}"/fix-cuda.patch
...
src_install() {
	...
	mkdir -p "${D}/usr/bin/b40c/"
	cp b40c/*.so "${D}/usr/bin/b40c/" 	|| die "Failed to install"
	cp *.ptx "${D}/usr/bin/" 		|| die "Failed to install"
